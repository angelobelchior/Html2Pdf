...
which is a command-line tool for converting HTML files to PDFs using the [QT Webkit](https://wiki.qt.io/Qt_WebKit) rendering engine.
...
First, we need to define the conversion parameters. For this, 
we use the [Arguments class](https://github.com/angelobelchior/Html2Pdf/blob/main/src/Arguments.cs).

These parameters are optional and can be configured as needed.

Basically, they are the same parameters that wkhtmltopdf accepts. 
If you want to know more, check out the [documentation](https://wkhtmltopdf.org/usage/wkhtmltopdf.txt).
...
Sample From an HTML template using Razor Syntax
-----------------------------------------------
...
Sample From an HTML template using Razor Syntax in Batch
--------------------------------------------------------
